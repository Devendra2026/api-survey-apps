...
| Infra images  | Postgres **17**, Redis **8**, MinIO from **`coollabsio/minio`** (pinned RELEASE + digest; includes `mc` for bucket init). Official Quay/Docker Hub `minio/*` anonymous pulls fail — do not use `:latest` |
...
5. Secrets in Dokploy **Environment** UI — paste [`deploy/env/dokploy.compose.env.example`](deploy/env/dokploy.compose.env.example) (replace `REPLACE_ME_*`). Dokploy writes `.env`; compose loads it into migrate/api/worker/web.
   **Interpolation-required (no defaults):** `POSTGRES_PASSWORD`, `REDIS_PASSWORD`, `MINIO_ROOT_USER`, `MINIO_ROOT_PASSWORD`.
   Entrypoints build `DATABASE_URL` from `POSTGRES_*` (do not paste a conflicting URL for in-compose Postgres).
...
| `401 Unauthorized` pulling `quay.io/minio/mc` or `quay.io/minio/minio` | Quay disabled anonymous pull for MinIO orgs. Compose must use `coollabsio/minio:RELEASE.2025-10-15T17-29-55Z@sha256:…` (server + init). Do not add Quay registry credentials; do not use `:latest`.                                                                                   |
...
| containerd extract / Prisma write `no space left on device`            | Host/containerd disk (or inodes) exhausted during image extract — not a pnpm/Prisma schema bug. On the Dokploy host run `bash scripts/ops/dokploy-disk-diag.sh`, then `bash scripts/ops/dokploy-safe-prune.sh` (never volume prune). Redeploy only with ~≥15 GiB free on Docker Root. |
