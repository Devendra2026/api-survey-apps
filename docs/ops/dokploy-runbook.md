...
- Startup: `minio-init` waits for MinIO **healthy** (no `connection refused` spam); `migrate` is a one-shot (`node:24-bookworm-slim` + OpenSSL, healthcheck disabled) that logs `✓` steps for env → Postgres wait → `prisma migrate deploy` only (`No pending migrations to apply.` is success). Catalog seed is a **one-time manual** step — see [dokploy-env.md](./dokploy-env.md) § One-time catalog seed.
